...
class JobFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager) : void
    {
        ...
        for ($i=0; $i < 20; $i++) { 
            $jobExpiresAt = new Job();
        $jobExpiresAt->setCategory($manager->merge($this->getReference('category-programming')));
        $jobExpiresAt->setType('full-time');
        $jobExpiresAt->setCompany('Vasilona Company'.$i);
        $jobExpiresAt->setLogo('sensio-labs.gif');
        $jobExpiresAt->setUrl('http://www.sensiolabs.com/');
        $jobExpiresAt->setPosition('Web Developer');
        $jobExpiresAt->setLocation('Madrid, Spain');
        $jobExpiresAt->setDescription('You\'ve already developed websites with symfony and you want to work with Open-Source technologies. You have a minimum of 3 years experience in web development with PHP or Java and you wish to participate to development of Web 2.0 sites using the best frameworks available.');
        $jobExpiresAt->setHowToApply('Send your resume to fabien.potencier [at] sensio.com');
        $jobExpiresAt->setPublic(true);
        $jobExpiresAt->setActivated(true);
        $jobExpiresAt->setToken('job_'.$i);
        $jobExpiresAt->setEmail('job@example.com');
        $jobExpiresAt->setExpiresAt(new \DateTime('30 days'));
        $manager->persist($jobExpiresAt);

        }

        for ($i=30; $i < 60; $i++) { 
            $jobExpiresAt = new Job();
        $jobExpiresAt->setCategory($manager->merge($this->getReference('category-design')));
        $jobExpiresAt->setType('full-time');
        $jobExpiresAt->setCompany('Vasilona Company'.$i);
        $jobExpiresAt->setLogo('sensio-labs.gif');
        $jobExpiresAt->setUrl('http://www.sensiolabs.com/');
